...
setwd('C:/Users/xjw5/OneDrive - Northern Arizona University/NAU/BNZ creek/RSN')
setwd('C:/Users/xanth/OneDrive - Northern Arizona University/NAU/BNZ creek/RSN')
library(dplyr)

bnz=read.csv(file="MasterInventory10.20.23.csv")
...
######### merge 2018 adn 2013 - keepign all 2018 and only unique from 2013
library(data.table)
...
# Don't do anythign after this beacuse it will take forever for the code to run 

#### look at tree trends over time
```{r bnz}

names(bnz)
levels(as.factor(bnz$SITE))
str(bnz)
bnz$year=as.numeric(bnz$year)

########### need to get rid of all the negative DBHs

sites = unique(bnz$SITE)

bnz=subset(bnz, SPECIES=="BETNEO"|SPECIES=="LARLAR"|SPECIES=="PICEA"|SPECIES=="PICGLA"|SPECIES=="PICMAR"|SPECIES=="POPBAL"|SPECIES=="POPTRE")

bnz$SPECIES= as.factor(bnz$SPECIES)
species = unique(bnz$SPECIES)
plot=unique(bnz$PLOT)
...
###########create a unique idea for site, plot, species
bnz=mutate(bnz, unique=paste(SITE, PLOT, SPECIES, sep = '_'))
head(bnz)
str(bnz)
unique=unique(bnz$unique)
 ##### this one 

for (i in unique) { 
  g = ggplot(data= subset(bnz, unique == i), aes(x=year, y=DBH.new, group=TREE, color=TREE, label=TREE))+geom_point(size=0) + geom_line(size=0.1) + geom_text(size=2)+ theme(legend.position="none")+ggtitle(i)

  ggsave(g, file=paste0(i,  ".png"), width = 14, height = 10, units = "cm")
  }


######## if a site, plot, species looks weird and you want to explore the tree number more closely use this code  
g=ggplot(subset(bnz, unique=="UP2A_21_BETNEO"), aes(x=year, y=DBH.new, group=TREE))
g=g+facet_wrap(~TREE, nrow=3) + theme(legend.position="none")
g=g+theme_bw()+geom_point()+geom_line()
g
